...
def get_embedder():
    ...
    if _embedder is None:
        ...
        # Read embedder backend from metadata to match the one used at build time
        meta = get_store().get_metadata()
        backend = meta.get("embedder", "local")
        _embedder = _get(backend)
    return _embedder
